...
class Acquisition:
    def __init__(
        ...
        if self.verbose > 0:
            ...
            print(
                (
                    "Send SIGUSR1 to dump intermediate results file: $ kill -SIGUSR1 %i"
                    % os.getpid()
                )
            )
            ...
            if len(goodpairs) > 1:
                ...
                with open(trsfile, "wb") as trs:
                    ...
                    for (iblock, oblock) in goodpairs:
                        # crypto data
                        trs.write(
                            iblock.to_bytes(self.blocksize, "big")
                            + oblock.to_bytes(self.blocksize, "big")
                        )
                tracefiles[mode == "dec"].append(trsfile)
        return tracefiles

    def doit(
        self, table, processed_input, protect=True, init=False, lastroundkeys=None
    ):
        ...
        if self.debug:
            print(
                (
                    ...
                    + input_stdin.hex()
                    + '"|xxd -r -p|'
                    + " ".join([self.targetbin] + input_args)
                )
            )
        try:
            if self.tolerate_error:
                proc = subprocess.Popen(
                    " ".join([self.targetbin] + input_args) + "; exit 0",
    ...
    def inject(self, r, faultfct):
        return (
            self.goldendata[: r[0]]
            + bytes([faultfct(x) for x in self.goldendata[r[0] : r[1]]])
            + self.goldendata[r[1] :]
        )
    ...
    def dig(self, tree=None, faults=None, level=0, candidates=[]):
        ...
        while len(tree) > 0:
            ...
            if self.start_from_left:
                ...
                if not self.depth_first_traversal:
                    if (
                        breadth_first_level_address is not None
                        and r[0] < breadth_first_level_address
                    ):
                        level += 1
                ...
                if not self.depth_first_traversal:
                    if (
                        breadth_first_level_address is not None
                        and r[1] > breadth_first_level_address
                    ):
                        level += 1
            ...
            if status in [self.FaultStatus.NoFault, self.FaultStatus.MinorFault]:
                ...
                self.FaultStatus.GoodEncFault,
                self.FaultStatus.GoodDecFault,
            ]:
                if (
                    status is self.FaultStatus.GoodEncFault
                    and self.minfaultspercol is not None
                    and self.encstatus[index] >= self.minfaultspercol
                ):
                    continue
                if (
                    status is self.FaultStatus.GoodDecFault
                    and self.minfaultspercol is not None
                    and self.decstatus[index] >= self.minfaultspercol
                ):
                    continue
                    ...
                    if isinstance(faults, list) and len(faults) > 1:
                        ...
                        while len(mycandidates) > 0:
                            ...
                            if status is self.FaultStatus.GoodEncFault:
                                ...
                                if (
                                    self.minfaultspercol is not None
                                    and [
                                        x
                                        for x in self.encstatus
                                        if x < self.minfaultspercol
                                    ]
                                    == []
                                ):
                                    return True
                ...
                self.FaultStatus.MajorFault,
                self.FaultStatus.Loop,
                self.FaultStatus.Crash,
            ]:
    ...
    def run(self, lastroundkeys=[], encrypt=None):
        ...
        starttime = time.time()
        oblock, status, index = self.doit(
            self.goldendata, self.processed_input, protect=False, init=True
        )
        ...
        if oblock is None or status is not self.FaultStatus.NoFault:
            raise AssertionError(
                "Error, could not obtain golden output, check your setup!"
            )
    ...
    def runoninput(self, lastroundkeys=[], encrypt=None, mimiclastround=True):
        ...
        processed_input = self.processinput(iblock, self.blocksize)
        oblock, status, index = self.doit(
            self.goldendata, processed_input, protect=False, init=True
        )
